...
def run_extraction(extract_func,file_gen,core_count=1,SaveBatchSize=25,output_dir=None):
    counter =0 
...
def execute(
    ...
    if os.path.isfile(pickle_file):
        ...
        filelist = Path(dicom_home).rglob("*.dcm")
    ...
    t_start = time.time()
    run_extraction(extractor,filelist,core_count=core_count,SaveBatchSize=SaveBatchSize,output_dir=output_directory)
